...
"""
convert a move to direction and step(example: R5 -> R(right) and 5(steps)) -> return a list of converted path
"""
def convert(path):
    new_path = list()
    path = path.split(",")
...
""" 
calculate move history of path
"""
def cal(paths):
    move_history = list()
    init_x = 0
    ...
    for path in paths:
        if path[0] == "L":
            for x in range(init_x, init_x - int(path[1]) - 1, -1):
                if[x, init_y] != [init_x, init_y]:
                    move_history.append([x, init_y])
            init_x = init_x - int(path[1])
        elif path[0] == "R":
            for x in range(init_x, init_x + int(path[1]) + 1):
                if[x, init_y] != [init_x, init_y]: 
                    move_history.append([x, init_y])
            init_x = init_x + int(path[1])
        elif path[0] == "U":
            for y in range(init_y, init_y + int(path[1]) + 1):
                if[init_x, y] != [init_x, init_y]:
    ...
    return move_history
"""
compare to path for finding equal value(intersection of 2 paths)
"""
...
"""
return min distance from intersections to origin of coordinates(O(0,0))
"""
def distance(intersections):
    dis = list()
    for intersection in intersections:
        dis.append(abs(intersection[0])+ abs(intersection[1]))
    return min(dis)
