...
def pre_cal(path):
    ...
    while i < len(path):
        move = path[i][0]
        incr = path[i][1]
        init_x, init_y, path_ = cal(init_x, init_y, move, incr, path_)
        i+=1
    return path_
def distance(dis):
    return(dis[0] + dis[1])
...
def cal(init_x, init_y, move, incr, path):
    if(move[0] == "L"):
        for i in range(init_x, init_x - int(incr) - 1, -1):
            path.append([i, init_y])
        return(init_x - int(incr), init_y, path)
    elif(move[0] == "R"):
        for i in range(init_x, init_x + int(incr) + 1):
            path.append([i, init_y])
        return(init_x + int(incr), init_y, path)
    elif(move[0] == "U"):
        for i in range(init_y, init_y + int(incr) + 1):
            path.append([init_x, i])
        return(init_x, init_y + int(incr), path)
    elif(move[0] == "D"):
        for i in range(init_y, init_y - int(incr) - 1, -1):
            path.append([init_x, i])
        return(init_x, init_y - int(incr), path)

def convert(path, path_):
    path = path.split(",")
    for items in path:
...
def main():
    ...
    path2 = convert(path_2, path2)
    """for i in pre_cal(path1):
        for j in pre_cal(path2):
            if i == j and i != [0, 0]:
                intersection.append(distance(i))
