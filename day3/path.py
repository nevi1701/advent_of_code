        ...
        if path[0] == "L":
            for x in range(init_x, init_x - int(path[1]) - 1, -1):
                if [x, init_y] != [init_x, init_y]:
                    path_arr = np.append(path_arr, [[x, init_y]], axis=0)
            ...
            for y in range(init_y, init_y + int(path[1]) + 1):
                if [init_x, y] != [init_x, init_y]:
                    path_arr = np.append(path_arr, [[init_x, y]], axis=0)
...
def distance(paths):
    dis = []
    for path in paths:
        dis.append(abs(path[0]) + abs(path[1]))
    return min(dis)
...
def main():
    ...
    path1 = np.unique(cal(convert(path1)), axis=0)
    path2 = np.unique(cal(convert(path2)), axis=0)
    x = set([tuple(x) for x in path1])
    y = set([tuple(x) for x in path2])
    z = x.intersection(y)
    z.remove((0, 0))
    print(distance(z))
